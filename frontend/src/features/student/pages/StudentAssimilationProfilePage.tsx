...
export default function StudentAssimilationProfilePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-display font-light text-navy">{t("student.profile.title").split(" ").slice(0, 2).join(" ")} <span className="italic text-orange">{t("student.profile.title").split(" ").slice(2).join(" ")}</span></h1>
        <p className="text-gray text-sm mt-1">{t("student.profile.subtitle")}</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-black/5">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-2xl font-bold text-red-600">{stats.remediation}</p><p className="text-xs text-gray">{t("student.profile.remediation")}</p></div>
          </div>
        </div>
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-black/5">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-blue-100 rounded-xl"><Minus className="w-5 h-5 text-blue-600" /></div>
            <div><p className="text-2xl font-bold text-blue-600">{stats.standard}</p><p className="text-xs text-gray">{t("student.profile.standard")}</p></div>
          </div>
        </div>
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-black/5">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-green-100 rounded-xl"><TrendingUp className="w-5 h-5 text-green-600" /></div>
            <div><p className="text-2xl font-bold text-green-600">{stats.avance}</p><p className="text-xs text-gray">{t("student.profile.advanced")}</p></div>
          </div>
        </div>
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-black/5">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-gray-100 rounded-xl"><Brain className="w-5 h-5 text-gray-600" /></div>
            <div><p className="text-2xl font-bold text-gray-600">{stats.default}</p><p className="text-xs text-gray">{t("student.profile.default")}</p></div>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        <div className="text-center py-12 text-gray">{t("student.profile.loading")}</div>
      ) : (
        <div className="space-y-4">
              ...
              <div key={ch.id} className="bg-white rounded-2xl shadow-sm border border-black/5 p-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray">{matiereName} • {notionCount} {t("student.profile.notionsCount")}</p>
                  </div>
                  <div className="text-end">
                    ...
                    {profile?.source === "defaut_matiere" && (
                      <p className="text-xs text-gray mt-1">{t("student.profile.defaultMatiere")}</p>
                    )}
                    {profile?.score_declencheur != null && (
                      <p className="text-xs text-gray mt-1">{t("student.profile.score")}: {Math.round(profile.score_declencheur * 100)}%</p>
                    )}
                  </div>
                </div>
              </div>
          ...
          {filteredChapters.length === 0 && (
            <div className="text-center py-12 text-gray">{t("student.profile.noChapters")}</div>
          )}
        </div>
      )}
    </div>
