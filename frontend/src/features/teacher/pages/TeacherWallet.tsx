...
export default function TeacherWallet() {
  ...
  const poolLabel = (p: string) =>
    ({ trial: t('teacher.wallet.pools.trial'), subscription: t('teacher.wallet.pools.subscription'), school_allocated: t('teacher.wallet.pools.school'), purchased: t('teacher.wallet.pools.purchased'), dt_purchased: t('teacher.wallet.pools.dtPurchased') })[p] || p;
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-black/5">
        <h1 className="text-3xl font-[300] text-navy">
          {t('teacher.wallet.title')} <span className="italic text-orange">{t('teacher.wallet.titleSuffix')}</span>
        </h1>
        <p className="text-gray mt-2">{t('teacher.wallet.subtitle')}</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-orange-p to-cream rounded-2xl p-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-gray">{t('teacher.wallet.aiCredits')}</span>
          </div>
        ...
        <div className="bg-gradient-to-br from-yellow-50 to-cream rounded-2xl p-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-gray">{t('teacher.wallet.tokens')}</span>
          </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/5">
        <h2 className="text-lg font-semibold text-navy mb-4">{t('teacher.wallet.detailBySource')}</h2>
                ...
                <div>
                  ...
                  {data.expires_at && (
                    <span className="ml-2 text-xs text-gray flex items-center gap-1 inline-flex">
                      ...
                      {t('teacher.wallet.expiresOn')} {new Date(data.expires_at).toLocaleDateString("fr-TN")}
                    </span>
                  )}
                </div>
                <div className="text-end">
                  <span className="font-semibold text-navy">{data.balance} {t('teacher.wallet.credits')}</span>
                </div>
              </div>
            );
          })}
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/5">
        <h2 className="text-lg font-semibold text-navy mb-4">{t('teacher.wallet.recentHistory')}</h2>
        {loading ? (
          <div className="text-center py-8 text-gray">{t('teacher.wallet.loading')}</div>
        ) : history.length === 0 ? (
          <div className="text-center py-8 text-gray">{t('teacher.wallet.noTransactions')}</div>
        ) : (
