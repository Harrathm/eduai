...
export default function PedagogicalLeadPage() {
  ...
  const reviewLocal = async (courseId: number, action: "approve_local" | "reject") => {
    if (!token) return;
  ...
  const escalateToPlatform = async (courseId: number) => {
    if (!token) return;
    setProcessing(courseId);
    try {
  ...
  if (user?.role !== "pedagogical_lead") {
    return (
      <div className="space-y-6">
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-black/5">
          <h1 className="text-3xl font-[300] text-navy">
            Responsable <span className="italic text-orange">Pédagogique</span>
          </h1>
        </div>
        <div className="bg-white rounded-2xl p-12 shadow-sm border border-black/5 text-center">
          ...
          <p className="text-gray-500">Cette page est réservée aux responsables pédagogiques d'école.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray">Enseignants</p>
                <p className="text-2xl font-bold text-navy">{performance.total_teachers}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray">Cours actifs</p>
                <p className="text-2xl font-bold text-navy">{performance.active_courses}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray">Taux complétion</p>
                <p className="text-2xl font-bold text-navy">{performance.completion_rate}%</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-black/5 flex items-center gap-4">
        ...
        <select value={filter} onChange={e => setFilter(e.target.value)}
          ...
          <option value="approved_for_platform">Approuvés</option>
